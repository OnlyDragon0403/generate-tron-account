...
library GlobalPlacementLib {
        ...
        view returns (bool)
    {
        ...
        for (index = 1; index <= ConstantLib._mem_cnt; index++) {
            if(self.list[ConstantLib.HEAD][index] != ConstantLib.HEAD)       // refers to range from 1 until Constant._mem_cnt for HEAD's childnodes
                return true;
        }
        return false;
    }
        ...
        view returns (bool)
    {
        ...
        if(self.list[_node][ConstantLib.PARENT] != ConstantLib.HEAD)
            return true;
        else
            for (index = 1; index <= ConstantLib._mem_cnt; index++) {
                if(self.list[ConstantLib.HEAD][index] == _node){
                    return true;
                }
            }
        return false;
    ...
    function sizeOf(SpaceMatrixLib.SpaceMatrix storage self) internal view returns (uint256 numElements) {
        ...
        for(nI = 1; nI <= ConstantLib._mem_cnt; nI++){
            uint nJ;
            (exists,i) = getNode(self, ConstantLib.HEAD, nI);
            while (i != ConstantLib.HEAD) {
                for(nJ = 1; nJ <= ConstantLib._mem_cnt; nJ++){
    ...
    function getParentNode(SpaceMatrixLib.SpaceMatrix storage self, uint256 _node)
        internal view returns (bool,uint256)
    {
        if (self.list[_node][ConstantLib.PARENT] == ConstantLib.HEAD && _node == 0) {
            return (false,0);
        } else {
            return (true, self.list[_node][ConstantLib.PARENT]);
        }
    }
    ...
    function createLink(SpaceMatrixLib.SpaceMatrix storage self, uint256 _node, uint256 _link, uint _nIndex) internal  {
        ...
        self.list[_link][ConstantLib.PARENT] = _node;
    }
    ...
    function getPrevNodeAndIndex(SpaceMatrixLib.SpaceMatrix storage self, uint256 _number, uint _level) internal view returns (uint256, uint256) {
        uint256 prevNode = ConstantLib.HEAD;
        uint256 nodeIndex;
        uint256 div;
        uint256 rest;
        uint deep = 0;
        ...
        while(deep != _level - 1){              // search prevNode and nodeIndex from company matrix
            uint256 nDiv = ConstantLib._mem_cnt ** (_level - deep - 1);
            rest = div % nDiv;
            div = div / nDiv;
            if( deep < _level - 1){
                nodeIndex = rest == 0 ? ( div == 0 ? ConstantLib._mem_cnt : div ) : div + 1;
                prevNode = self.list[prevNode][nodeIndex];
            }

            deep++;
            if(deep == _level - 1)
                nodeIndex = rest == 0 ? ConstantLib._mem_cnt : rest;
            div = rest;
        }
    ...
    function getGlobalPlacement(SpaceMatrixLib.SpaceMatrix storage self) internal returns (uint256, uint256 , uint256) {         // onwards level4 placement
        ...
        lv1Limit = ConstantLib._mem_cnt ** self._arrangement[0];
        lv2Limit = ConstantLib._mem_cnt ** ( self._arrangement[0] + 1 );
    ...
    function getLevelNumbers(SpaceMatrixLib.SpaceMatrix storage self , uint256 node) internal view returns (uint256, uint256, uint256){
        ...
        for(i = 0; i < ConstantLib._mem_cnt; i++){
            if(self.list[node][i + 1] == ConstantLib.HEAD)
        ...
        for(i = 0; i < ConstantLib._mem_cnt; i++){
            ...
            for(j = 0; j < ConstantLib._mem_cnt; j++){
                if(self.list[cur_node][j + 1] == ConstantLib.HEAD)
                    return (lv_num_1, lv_num_2, lv_num_3);
        ...
        for(i = 0; i < ConstantLib._mem_cnt ** 2; i++){
