...
library DonationLib {
    ...
    function getAddrOfMemberOwner(
        ...
        if(cur_level != member_level){        // send unclaimed money to unclaimed wallet from ConstantLib
            ...
            if( member_level == 1 ){
                main_wallet._unclaimed_wallet += space_price[space_ID] / 20 * ConstantLib._rate;
                sendTRXToUnclaimedWallet(src_addr , ConstantLib._unclaimed_Wallet ,space_price[space_ID] * 3 / 4 );
            }

            if( member_level == 2 ){
                main_wallet._unclaimed_wallet += space_price[space_ID] / 5 * ConstantLib._rate;
                sendTRXToUnclaimedWallet(src_addr , ConstantLib._unclaimed_Wallet ,space_price[space_ID] * 7 / 10 );
            }

            if( member_level == 3 ){
                main_wallet._unclaimed_wallet += space_price[space_ID] / 2 * ConstantLib._rate;
                sendTRXToUnclaimedWallet(src_addr , ConstantLib._unclaimed_Wallet, space_price[space_ID] / 2 );
            }
            return false;
        }
        else{
            ...
            if(member_level == 2){
                ...
                if(member_pos >= 21 && member_pos <= 25){       // send money to re-entry wallet
                    ...
                    self.fnDisplay( wallet[self._ID_member_group[parent]]._re_entry_wallet[entry_ID - 1] );
                    while( wallet[self._ID_member_group[parent]].CheckReenterWallet(entry_ID) ){
                        ...
                        self.enterSpecificSpace(entry_ID , self._ID_member_group[parent]);
                    }
                    return false;
                }
                ...
                if(member_pos >= 1 && member_pos <= 11){         // send money to upgrade wallet
                    if(self._space_ID == 5){
                        ...
                        if( wallet[self._ID_member_group[parent]].CheckUpgradeWallet(space_ID) ){
                            ...
                            self.enterNextSpace(self._ID_member_group[parent]);
                        }
                    }
                    return false;
                }
    ...
    function matrixDonationNthLevel (
        ...
        bool err;
        err = self.getAddrOfMemberOwner(wallet , main_wallet , src_addr, level, member_pos);
    }
    ...
    function sendDirectTRX (
        ...
        _transfer(src_addr, owner_addr, amount * ConstantLib._rate / 100);
    }
    ...
    function sendTRXToUnclaimedWallet(
        ...
        _transfer(src_addr, dest_addr, amount * ConstantLib._rate / 100);
    }
    ...
    function sendTRXToCompanyWallet(
        ...
        _transfer(owner_addr, ConstantLib._company_Wallet_1, space_price[self._space_ID] * ConstantLib._rate / 100);
        main_wallet._company_wallet[0] += space_price[self._space_ID] * ConstantLib._rate / 100;
        _transfer(owner_addr, ConstantLib._company_Wallet_2, space_price[self._space_ID] * ConstantLib._rate / 100);
        main_wallet._company_wallet[1] += space_price[self._space_ID] * ConstantLib._rate / 100;
        _transfer(owner_addr, ConstantLib._company_Wallet_3, space_price[self._space_ID] * ConstantLib._rate / 100);
        main_wallet._company_wallet[2] += space_price[self._space_ID] * ConstantLib._rate / 100;
    }
    ...
    function sendTRXToInvitedAccount(         // Invite Donation  20% money to invited address
        ...
        _transfer(owner_addr, invite_addr, space_price[self._space_ID] * ConstantLib._rate / 5);
    }
